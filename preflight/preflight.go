...
// Unsupported — единственная псевдо-проверка для ОС, где локальное
// управление ботом не поддерживается (см. botproc/manage_other.go — сам бот
// живёт только на Linux). Показывать здесь пять проваленных проверок под
// несуществующий локальный каталог только запутывало бы: единственный
// осмысленный следующий шаг на такой ОС — подключиться к боту по SSH.
func Unsupported() []Check {
	return []Check{
		{
