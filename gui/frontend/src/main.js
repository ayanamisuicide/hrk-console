...
function renderModules() {
    ...
    // Полная пересортировка на каждый тик читалась как дёрганье — не из-за
    // самого движения, а из-за настоящего бага (см. коммит "не трогает DOM
    // строк модулей, если позиция и так верна"): appendChild на КАЖДОЙ
    // строке каждый тик физически выдёргивал её из DOM и вставлял обратно,
    // даже когда позиция не менялась, и это перезапускало анимацию
    // появления у всего списка разом. Тот баг пофикшен — DOM трогается
    // только когда позиция реально меняется, — так что живая пересортировка
    // теперь безопасна: по одному обмену соседних строк за тик (самая
    // рассогласованная пара по разнице счётчиков), а не всё сразу. Дальний
    // подъём (был 20-м, стал первым) займёт несколько тиков, каждый —
    // заметный, чистый обмен местами двух строк с FLIP-анимацией.
    // Пока панель закрыта — не пересчитываем: смысла её тревожить, пока
    // никто не смотрит, нет (см. resortModOrder — полная сортировка при
    // открытии, отдельно).
    ...
    if (modMenu.classList.contains('visible')) {
