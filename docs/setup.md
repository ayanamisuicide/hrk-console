...
`launch.sh` открывает новое окно терминала с выбором «Консоль (hkc) / Приложение (GUI)» и сама собирает то, что выбрано, если оно ещё не собрано: `make build` для консоли, `make gui` для GUI — а если для GUI ещё и не хватает `wails`, ставит его тоже сама (`go install`, без `sudo`). Системные библиотеки webview (`libgtk-3-dev`, `libwebkit2gtk-4.1-dev`, …, см. [gui/README.md](../gui/README.md#зависимости)) — единственное, что скрипт сам не ставит: это требует `sudo`, и если из-за их отсутствия `make gui` всё же не соберётся, скрипт просто печатает нужную команду. Явный номер режима (`launch.sh 1`…`4`) по-прежнему пропускает этот выбор и сразу открывает консоль в нужном режиме — старое поведение, на него могут полагаться существующие ярлыки. В репозитории лежит `Heroku Logs.desktop` с плейсхолдером вместо пути:
