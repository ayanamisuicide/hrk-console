...
# По тексту — баннер версии, он же приезжает продолжением строки от root.
# "Reloaded N commands" сюда не годится, хотя и просится: бот пересобирает
# модули каждые 30 секунд, в живом логе это 2103 одинаковые записи, то есть
